            ...
            if (t.solid){
                ...
                if(getBoundsBottom().intersects(t.getBounds()) || getBoundsTop().intersects(t.getBounds())
                    ...
                    if(t.getId()==Id.diamante){//Diamante
                        ...
                        if(((objeto.Llave)t).isDisponible()){
                            //System.out.println("disponble");
                            if(x+width>t.x+36 && x<t.x+t.width-36 && y+height>t.y+36 && y<t.y+t.height-36){
                                //System.out.println("dentro");
                                ((objeto.Llave)t).setRecogido(true);
                                handler.nivel.puerta.setLocked(false);
                            }
                        }
                    }
                    if(t.getId()==Id.puerta && !((objeto.Puerta)t).locked &&
                            JuegoPlataforma.nivel<=JuegoPlataforma.nivelesMapeo.length){//Puerta
                        handler.nextLevel();
                    }
                }
            }
