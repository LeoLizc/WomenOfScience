...
public class KeyInput implements KeyListener {
    ...
    public boolean keys[]={false,false,false,false},truco=false;
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if(JuegoPlataforma.etapa==EtapaJuego.menúPrinc){
            ...
            if(handler.nivel.etapa==EtapaNivel.tranquiloDeLaVida){
                if(!JuegoPlataforma.pausado){
                    switch (key) {
                        ...
                        case KeyEvent.VK_SPACE:
                            if (keys[3] && (System.currentTimeMillis()-timeRef)<1000*combo) {
                                //System.out.println("GOOOOOOOOLLL!!!!");
                                javax.swing.JOptionPane.showMessageDialog(null, "Ahora Puedes Volar!!!!");
                                truco=!truco;
                            }else restart();
                            timeRef=System.currentTimeMillis();
                            break;
                ...
                if(key==KeyEvent.VK_ENTER)if (handler.nivel.cinematica.isAble()) {
                    System.out.println("Entra");
                    handler.nivel.nextScene();
                }else;else if(key==KeyEvent.VK_ESCAPE){
    ...
    public void keyReleased(KeyEvent e) {
        ...
        if(JuegoPlataforma.etapa==EtapaJuego.juego){
            switch (key) {
                case KeyEvent.VK_W:
                    //jug.setVelY(0);
                    if((jug.estado==Estado.jumping || jug.estado==Estado.falling && truco) 
                            && (jug.aceleración>5 || truco)){
